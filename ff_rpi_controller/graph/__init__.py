...
class Graph(Widget):
    ...
    # but is in the axis min_y - max_y range
    ...
    def _get_ticks(self, major, minor, log, s_min, s_max):
        if major and s_max > s_min:
            if log:
                s_min = log10(s_min)
                s_max = log10(s_max)
                # count the decades in min_y - max_y. This is in actual decades,
                # not logs.
    ...
    def get_x_axis(self, axis=0):
        ...
        return (info["log"], info["min_y"], info["max_y"])
    ...
    x_ticks_minor = BoundedNumericProperty(0, min_y=0)
    ...
    y_ticks_major = BoundedNumericProperty(0, min_y=0)
    ...
    y_ticks_minor = BoundedNumericProperty(0, min_y=0)
...
class Plot(EventDispatcher):
    ...
    def update(self, xlog, xmin, xmax, ylog, ymin, ymax, size):
        ...
        log, min_y, max_y indicate the axis settings.
        size a 4-tuple describing the bounding box in which we can draw
        graphs, it's (x0, y0, x1, y1), which correspond with the bottom left
        and top right corner locations, respectively.
        '''
...
class ContourPlot(Plot):
    ...
    The user must first specify 'xrange' and 'yrange' (tuples of min_y,max_y) and
    ...
    The color values are automatically scaled to the min_y and max_y z range of the
    data set.
    """
